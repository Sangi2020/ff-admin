...
class POSController extends Controller
{
    ...
    public function addToCart(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        // Updated Code start
        $discounted_product_price = $price - $discount;
        $taxOnProduct = Helpers::tax_calculate($product, $discounted_product_price);
        // Updated Code end
    ...
    public function placeOrder(Request $request): RedirectResponse
    {
        ...
        if ($orderType == 'home_delivery'){
            ...
            $deliveryCharge = Helpers::get_delivery_charge(branchId: session()->get('branch_id') ?? 1, distance:  $distance, selectedDeliveryArea: $areaId);
        ...
        foreach ($cart as $c) {
            if (is_array($c)) {
                ...
                if ($product) {
                    ...
                    $taxOnProduct = Helpers::tax_calculate($product, $priceAfterDiscount);
                    // Updated Code end 
        ...
        if ($orderType == 'home_delivery'){
            $productWeightCharge = Helpers::productWeightChargeCalculation(branchId: session('branch_id') ?? 1, weight: $productWeight);
        }
