...
@if(session()->has('cart') && count( session()->get('cart')) > 0)
    ...
    @foreach(session()->get('cart') as $key => $cartItem)
        @if(is_array($cartItem))
            ...
            $itemDiscount = ($cartItem['discount'] * $cartItem['quantity']);
            
            // Add to running totals
            $subtotal += $productSubtotal;
            $discountOnProduct += $itemDiscount;
            
            // Calculate discounted price for tax calculation
            $discountedPrice = $cartItem['price'] - $cartItem['discount'];
            
            // Get product and calculate tax on discounted price
            $product = \App\Model\Product::find($cartItem['id']);
            $itemTax = \App\CentralLogics\Helpers::tax_calculate($product, $discountedPrice) * $cartItem['quantity'];
            
            // Add to total tax based on VAT status
            $totalTax += $itemTax;
            $updatedTotalTax += $vatStatus === 'included' ? 0 : $itemTax;
            ...
            <tr>
                ...
                <td class="text-center px-0 py-1">
                    <div class="btn text-left">
                        {{ \App\CentralLogics\Helpers::set_symbol($productSubtotal) }}
                    </div>
                </td>
                <td>
...
if (session()->get('order_type') == 'home_delivery'){
    ...
    $deliveryCharge = \App\CentralLogics\Helpers::get_delivery_charge(branchId: session('branch_id') ?? 1, distance:  $distance, selectedDeliveryArea: $areaId);
    $productWeightCharge = \App\CentralLogics\Helpers::productWeightChargeCalculation(branchId: session()->get('branch_id') ?? 1, weight: $productWeight);
}else{
    $deliveryCharge = 0;
}
...
<div class="box p-3">
    <dl class="row">
        ...
        <dd class="col-sm-6 text-right h4 b">{{ Helpers::set_symbol(round($total+$updatedTotalTax+$deliveryCharge+$productWeightCharge, 2)) }}</dd>
    </dl>
    <div>
                ...
                <form action="{{route('admin.pos.discount')}}" method="post" class="row">
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <select name="type" class="form-control" id="discount_type_select">
                            <option value="amount" {{$extraDiscount_type=='amount'?'selected':''}}>{{translate('amount')}}
                                ({{\App\CentralLogics\Helpers::currency_symbol()}})
                            </option>
                            <option value="percent" {{$extraDiscount_type=='percent'?'selected':''}}>{{translate('percent')}}(%)
                            </option>
                        </select>
                    </div>
                    <div class="col-sm-12">
