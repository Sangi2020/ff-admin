...
<div class="content container-fluid">
    ...
    <div class="col-12">
        <div class="card">
            <form id="login-setup-form" action="{{route('admin.business-settings.store.login-setup-update')}}" method="post">
                @csrf
                ...
                <div class="btn--container mb-4  justify-content-end mr-3">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <div class="py-4">
                        <h5 class="modal-title" id="smsConfigModalLabel">{{ translate('Set Up SMS Configuration/Firebase OTP Verification First') }}</h5>
                    </div>
                    <p>{{ translate('It looks like your SMS configuration is not set up yet. To enable the OTP system, please set up the SMS configuration/Firebase OTP Verification first.') }}</p>
                </div>
    ...
    <div class="modal fade" id="socialMediaConfigModal" tabindex="-1" role="dialog" aria-labelledby="socialMediaConfigModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="text text-center mb-5">
                    <a href="{{route('admin.business-settings.web-app.third-party.social-media-login')}}" target="_blank" class="btn btn-primary">{{ translate('Go to Apple Configuration') }}</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function isSmsGatewayActivated(callback) {
                $.ajax({
                    url: '{{ route('admin.business-settings.store.check-active-sms-gateway') }}',
                    method: 'GET',
        ...
        $(document).ready(function() {
            function isSocialMediaActivated(socialMedia, callback) {
                $.ajax({
                    url: '{{ route('admin.business-settings.store.check-active-social-media') }}',
                    method: 'GET',
                    success: function(response) {
